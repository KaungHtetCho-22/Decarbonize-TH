...
if [ -z "$DOCKERHUB_USER" ] || [ -z "$DOCKERHUB_TOKEN" ]; then
  echo "DOCKERHUB_USER or DOCKERHUB_TOKEN not set"
  exit 1
fi
...
for SERVICE in "${!services[@]}"; do
  ...
  docker build \
    -t $DOCKERHUB_USER/$SERVICE:$GIT_TAG \
    -t $DOCKERHUB_USER/$SERVICE:latest \
    --file "./$CONTEXT/Dockerfile" \
    "./$CONTEXT"

  echo "Pushing $DOCKERHUB_USER/$SERVICE:$GIT_TAG and :latest..."
  docker push $DOCKERHUB_USER/$SERVICE:$GIT_TAG
...
for SERVICE in "${!services[@]}"; do
  ...
  echo "    image: $DOCKERHUB_USER/$SERVICE:$GIT_TAG" >> $OUTPUT_FILE
